...
💡 Note: Jump to the quick start [end to end reference](./etoe_reference_example.md) example to get up and running quickly. For a single-file technical walkthrough (setup, Elastic connection, rules, unit tests, schemas) without governance options, see the [DaC Quick Start Guide](./dac_quick_start_guide.md).
